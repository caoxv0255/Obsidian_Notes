...
### 高数题型补充（同济/托马斯）
- **先看收敛方式**：点收敛还是一致收敛，结论完全不同
- **常用工具**：M判别法、Dirichlet 判别法、Abel 判别法
- **常见套路**：把级数和函数联系起来，再用逐项运算

### 数分严谨补充（华师大/Rudin）
- **一致收敛是核心**：保证极限函数连续、可积与可交换运算
- **逐项求导更强**：需要导数级数一致收敛，不能只看原级数
...
$$
而 $\sum_{n=1}^{\infty} \frac{1}{n^2}$ 收敛，故由 M 判别法可知原级数在 $[-1,1]$ 上一致收敛。

因此其和函数连续，并可在闭区间内逐项积分。
