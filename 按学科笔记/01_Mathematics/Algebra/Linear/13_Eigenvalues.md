...
- 会从 $\det(A-\lambda I)=0$ 求特征值，并求对应特征向量
- 理解对称矩阵的谱性质（实特征值、正交对角化）
- 能把特征值性质（迹/行列式/谱半径）用于快速校验与估计

## 难度分层

- **基础**：定义 + 2×2/3×3 计算 + 迹/行列式关系
- **进阶**：对称矩阵谱定理与正交对角化
- **拓展**：Gershgorin 圆估计与数值稳定性直觉
...
1. 求矩阵 $\begin{bmatrix} a & b \\ b & a \end{bmatrix}$ 的特征值与特征向量。
2. 给定一个上三角矩阵，说明为什么其特征值是对角线元素。

### B档（辨析）

1. 给出一个矩阵例子：可对角化但不可正交对角化，并解释原因。
2. 解释几何意义：特征向量为什么“方向不变”？

### C档（证明/推导）

1. 证明：$\sum_i \lambda_i = \mathrm{tr}(A)$ 与 $\prod_i \lambda_i = \det(A)$。

## 自测（3问速测）

1. 我能快速判断一个矩阵“是否一定有实特征值”吗？（对称/非对称）
2. 我能用迹/行列式对结果做 sanity check 吗？
3. 我能解释谱定理与 SVD 的关系吗？
