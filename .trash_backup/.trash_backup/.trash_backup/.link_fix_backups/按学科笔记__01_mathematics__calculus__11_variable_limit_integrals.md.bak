...
### 高数题型补充（同济/托马斯）
- **变上限求导**：先写成积分上限函数，再套微积分基本定理
- **复合上下限**：链式法则和积分求导公式一起用
- **积分方程**：先对两边求导，再用初值确定常数

### 数分严谨补充（华师大/Rudin）
- **连续性来源**：可积函数的积分上限函数一定连续
- **可导性条件**：要求被积函数在点处连续，不能只靠可积
- **证明主线**：把“平均值”型差商改写成局部积分平均
...
若
$$
F(x)=\int_{x^2}^{\sin x} \!\!\! \phi(t)\,dt
$$
则
$$
F'(x)=\phi(\sin x)\cos x-2x\,\phi(x^2)
$$
