...
### 高数题型补充（同济/托马斯）
- **先选方法**：绕 $x$ 轴用圆盘/垫圈，绕 $y$ 轴常考虑圆柱壳
- **先用对称**：偶函数、对称区域、旋转体常能减半计算
- **常见应用**：面积、体积、弧长、功、质心、转动惯量

### 数分严谨补充（华师大/Rudin）
- **建模先于计算**：先确定区域、边界与变量方向，再写积分
- **函数符号要清楚**：面积用绝对值，体积常注意截面厚度

### 完整例题（圆柱壳法）
求由 $y=x$ 与 $y=x^2$ 围成的区域绕 $y$ 轴旋转所得体积。

解：在 $[0,1]$ 上，壳半径为 $x$，壳高为 $x-x^2$，故
$$
