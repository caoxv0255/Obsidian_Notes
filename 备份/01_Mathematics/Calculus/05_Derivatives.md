...
### 高数题型树（同济/托马斯）
- **直接法则型**：幂、指、对、三角及复合
- **隐函数与参数方程型**：先写关系，再对自变量求导
- **最值与单调型**：$f'(x)$ 符号表 + 临界点分类
- **综合计算型**：对数求导、分段函数可导性、高阶导数模式识别

### 数分重点（华师大/Rudin）
- **可导 $\Rightarrow$ 连续，但连续 $\nRightarrow$ 可导**
- **Darboux 性质**：导函数未必连续，但具有介值性
- **中值定理前提**：闭区间连续 + 开区间可导，缺一不可
...
理由：左导数 $f'_-(0)=-1$，右导数 $f'_+(0)=1$，左右不等。
...
解：
$$
f'(0)=\lim_{h\to0}\frac{h^2\sin(1/h)-0}{h}
=\lim_{h\to0}h\sin(1/h)=0
$$
所以在 0 可导，且 $f'(0)=0$。这类题要先回到导数定义，不要先套连续函数求导法则。
