...
### 高数补充（托马斯/高等数学）
- **计算导向**：快速写系数、利用奇偶性减少积分量
- **题型导向**：方波/锯齿波/绝对值函数的标准展开

### 数分补充（华师大/Rudin）
- **收敛点判定**：连续点收敛到函数值，第一类间断点收敛到左右极限均值
- **均方收敛视角**：在 $L^2$ 空间下，傅里叶级数是到闭子空间的正交投影

### 完整例题（奇函数展开）
在 $(-\pi,\pi)$ 上设 $f(x)=x$，其傅里叶级数为
$$
...
$$
思路：对对应偶函数展开或直接用 Parseval 恒等式，属于“级数求和 via Fourier”的经典桥梁题。
