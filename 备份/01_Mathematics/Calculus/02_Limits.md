...
- **题型 A：等价无穷小代换**
    $$
    \sin x \sim x,\ \tan x \sim x,\ 1-\cos x \sim \frac{x^2}{2},\ \ln(1+x) \sim x,\ e^x-1 \sim x
    $$
- **题型 B：无穷大量阶比较**
    当 $x \to \infty$：$\ln x \ll x^\alpha \ll a^x \ll x! \ll x^x$（$\alpha>0, a>1$）
- **题型 C：参数极限与夹逼**
    先证有界，再用单调或夹逼，避免直接套洛必达。

### 数分严谨补充（华师大/北大数分/Rudin）
- **证明模板 1：唯一性**
    反设同一点有两个不同极限 $A \neq B$，取 $\varepsilon = |A-B|/3$，由定义导出矛盾。
- **证明模板 2：四则运算**
    先把差值拆分，再用三角不等式与“有界性 + 收敛到 0”的乘积控制。
- **证明模板 3：复合函数极限**
    显式写出外函数在极限点连续所需条件，避免把“可复合”与“可连续延拓”混淆。
...
$$
\frac{e^{\sin x}-1}{x}
=\frac{e^{\sin x}-1}{\sin x}\cdot\frac{\sin x}{x}
\xrightarrow[x\to 0]{}1\cdot 1=1
$$

数分解释：第一因子是函数
$g(u)=\frac{e^u-1}{u}$
在 $u\to0$ 的极限（可由中值定理或泰勒证明为 1），第二因子为标准极限。
