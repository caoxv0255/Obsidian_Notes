# 3.5 实盘接口
## 模块导航
- [[MY_Learning/Year_3/01_Quant_Advanced/3.5_实盘接口/01_实盘接口基础]] ← 当前
- [[MY_Learning/Year_3/01_Quant_Advanced/3.5_实盘接口/02_数据管理与存储]]
- [[03_回测系统设计]]
...
flowchart TD
    subgraph DL["📊 数据层 Data Layer"]
        D1[Market Data<br/>行情数据]
        D2[Alternative Data<br/>替代数据]
        D3[News & Sentiment<br/>新闻舆情]
    end
    
    subgraph FL["🔬 因子层 Factor Layer"]
        F1[Alpha Factors<br/>Alpha因子]
        F2[Risk Factors<br/>风险因子]
        F3[Feature Engineering<br/>特征工程]
    end
    
    subgraph SL["🧠 策略层 Strategy Layer"]
        S1[Portfolio Construction<br/>组合构建]
        S2[Risk Management<br/>风险管理]
        S3[Order Generation<br/>信号生成]
    end
    
    subgraph EL["⚡ 执行层 Execution Layer"]
        E1[Broker API<br/>券商接口]
        E2[Order Management<br/>订单管理]
        E3[Transaction Costs<br/>交易成本]
    end
